...
const PeopleItem = ({ user }) => {
    const { image, name, aboutShort, lastPostDate } = user;
    return (
        <div class="media userContainer">
            ...
            <div className="media-body p-1">
                ...
                <span className="w-75 d-inline-block pl-5">{aboutShort}</span><span className="w-25 d-inline-block pl-5">Last post:{user.getPostTime()}</span>
            </div>
        </div>
    )
